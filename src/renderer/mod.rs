...
impl Renderer {
    ...
    fn render_message(
        ...
        for (g, group) in message.groups.into_iter().enumerate() {
            ...
            while let Some((i, section)) = message_iter.next() {
                let peek = message_iter.peek().map(|(_, s)| s).cloned();
                match &section {
                    Element::Title(title) => {
                        self.render_title(
                            buffer,
                    ...
                    Element::Cause(cause) => {
                            ...
                            source_map_annotated_lines.pop_front()
                        {
                            ...
                            if cause.col_separator.unwrap_or(false)
                                || (cause.col_separator.is_none() && g == 0 && group_len > 1)
                            {
                                ...
                                } else if !(cause.col_separator.is_none() && peek.is_some()) {
    ...
    fn render_title(
        ...
        let mut line_offset = buffer.num_lines();
        if title.needs_col_separator {
            self.draw_col_separator_no_space(buffer, line_offset, max_line_num_len + 1);
            line_offset += 1;
        }

        let (has_primary_spans, has_span_labels) = next_section
            .map(|s| match s {
                Element::Title(_) => (false, false),
                Element::Cause(cause) => (
    ...
    fn render_snippet_annotations(
        ...
        sm: &SourceMap<'_>,
        annotated_lines: Vec<AnnotatedLineInfo<'_>>,
    ...
    fn emit_suggestion_default(
        ...
        let mut row_num = buffer_offset + (!is_cont) as usize;
