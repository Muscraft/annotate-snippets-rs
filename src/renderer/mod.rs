...
use crate::{Annotation, AnnotationKind, Level, Message, Patch, Section, Snippet, Title};
pub use anstyle::*;
...
impl Renderer {
    ...
    fn render_message(
        ...
        while let Some((i, section)) = sections_iter.next() {
            let peek = sections_iter.peek().map(|(_, s)| s).cloned();
            match section {
                Section::Title(title) => {
                    if i != 0 && !children_started {
                        match &peek {
                            None | Some(Section::Title(_)) => {
                    ...
                    self.render_title(
                        ...
                        matches!(peek, Some(Section::Title(_)))
                            && matches!(
                                message.sections.get(i + 2),
                                None | Some(Section::Title(_))
                            ),
                    );
                }
    ...
    fn render_snippet_annotations(
        ...
        for annotated_line_idx in 0..annotated_lines.len() {
            ...
            let depths = self.render_source_line(
                ...
                max_line_num_len,
                margin,
                close_window && annotated_line_idx + 1 == annotated_lines.len(),
            );
    ...
    fn render_source_line(
        ...
        if line_info.annotations.iter().all(|a| a.is_line()) {
            return vec![];
        }
    ...
    fn emit_suggestion_default(
        ...
        let mut row_num = buffer_offset + 1;
        for (i, (complete, parts, highlights)) in suggestions.iter().enumerate() {
            ...
            if let (Some(primary_origin), Some(origin)) = (primary_origin, suggestion.origin) {
                ...
                if origin != primary_origin {
                    ...
                    if row_num == 2 {
                        ...
                        buffer.puts(1, col, &message, ElementStyle::LineAndColumn);
                    } else {
            ...
            let start = parts.iter().map(|p| p.range.start).min().unwrap();
            let end = parts.iter().map(|p| p.range.end).max().unwrap();
            let file_lines = sm.span_to_lines(start..end);
            let (line_start, line_end) = sm.span_to_locations(start..end);
    ...
    fn draw_line(
        ...
        // Was originally Style::Quotation
