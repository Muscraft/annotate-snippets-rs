...
impl<'a> SourceMap<'a> {
    ...
    pub(crate) fn span_to_locations(&self, span: Range<usize>) -> (Loc, Loc) {
        let start_info = self
            ...
            .iter()
            .find(|info| {
                span.start >= info.start_byte
                    && (span.start < info.end_byte
                        || (span.start == info.end_byte && span.start == span.end))
            })
            .unwrap_or(self.lines.last().unwrap());
        let (start_char_pos, start_display_pos) = start_info.line
    ...
    pub(crate) fn annotated_lines(
        ...
        for ann in multiline_annotations.clone() {
            for a in multiline_annotations.iter_mut() {
        ...
        for ann in multiline_annotations {
            ...
            if !ann.overlaps_exactly {
                ...
                let until = (ann.start.line..middle)
                    .rev()
                    .filter_map(|line| self.get_line(line - 1).map(|s| (line + 1, s)))
                    .find(|(_, s)| filter(s))
                    .map(|(line, _)| line)
                    .unwrap_or(ann.start.line);
                ...
                let end_is_empty = self.get_line(line_end - 1).map_or(false, |s| !filter(s));
